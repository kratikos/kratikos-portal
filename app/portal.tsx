...
function Dashboard() {
  ...
  const topStates = dashboard?.usersByState.slice(0, 5) ?? [];
  ...
  return (
    <div className="page-stack">
        ...
        <article className="panel map-panel">
          ...
          <div className="map-content">
            <div className="brazil-map" aria-label="Mapa ilustrativo do Brasil">
              ...
              {topStates.slice(0, 3).map((item, index) => <div className={`map-callout ${index === 0 ? "callout-sp" : index === 1 ? "callout-ba" : "callout-rj"}`} key={item.state}><b>{item.state}</b><span>{number(item.total)}</span></div>)}
            </div>
            <div className="state-list">
              {topStates.length ? topStates.map((item, index) => <div key={item.state}><span>{index + 1}</span><b>{item.state}</b><div><i style={{ width: `${(item.total / maxStateUsers) * 100}%` }} /></div><strong>{number(item.total)}</strong></div>) : <p className="muted">Sem localização informada.</p>}
            </div>
          </div>
        </article>
      </section>
      ...
      <section className="panel top-panel">
        <div className="panel-heading"><div><span className="section-kicker">Rankings · integração pendente</span><h2>Top 10 da plataforma</h2></div><button className="text-button">Exportar relatório ↗</button></div>
